...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo
{
    ...
    //add qiulei.wang [url.absoluteString length]<1 确保url不是空串，如果url为空串，会导致内存泄漏，不是一个正确格式的url也会导致泄漏。
    if (!url || !delegate || (!(options & SDWebImageRetryFailed) && [failedURLs containsObject:url])||[url.absoluteString length]<1)
    {
        return;
    }
...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
	...
	//add by erlangZhang
	if ((options & SDWebImageProgressDisplay) && !downloader.progressDisplay)
	{
		// Turn progressDisplay download support on demand
		downloader.progressDisplay = YES;
	}
...
//add by erlangZhang
- (void)imageDownloader:(SDWebImageDownloader *)downloader totalSize:(NSUInteger)totalSize receiveSize:(NSUInteger)receiveSize
{
	// Notify all the downloadDelegates with this downloader
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
		{
			...
			SDWIRetain(delegate);
            SDWIAutorelease(delegate);
			if ([delegate respondsToSelector:@selector(webImageManager:totalSize:receiveSize:)])
			{
				[delegate webImageManager:self totalSize:totalSize receiveSize:receiveSize];
			}
		}
	}
}

#pragma mark - add by erlangZhang

- (BOOL)FileIsInDiskWithURLStr:(NSString*)url
{
    if ([url length] == 0) {
        return NO;
    }
    NSURL* nsurl = [NSURL URLWithString:url];
    return [self FileIsInDiskWithURL:nsurl];
}

- (BOOL)FileIsInDiskWithURL:(NSURL *)url
{
    if (url == nil) {
        return NO;
    }
    NSString* key = [self cacheKeyForURL:url];
    return [[SDImageCache sharedImageCache] imageExistInDisk:key];
}
